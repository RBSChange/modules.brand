...
class brand_BrandService extends f_persistentdocument_DocumentService
{
	...
	{
		$query = $this->createQuery()->add(Restrictions::published())->add(Restrictions::eq('id', $brand->getId()))->setProjection(Projections::count('id', 'count'));
		
		$query->createPropertyCriteria('brandId', 'modules_catalog/compiledproduct')->add(Restrictions::published())->add(Restrictions::eq('websiteId', $website->getId()));
	...
	{
		$query = $this->createQuery()->add(Restrictions::published())->setProjection(Projections::distinctCount('id', 'count'));
	...
	{
		$query = $this->createQuery()->addOrder(Order::asc('document_label'))->add(Restrictions::published());
	...
	{
		$query = $this->createQuery()->addOrder(Order::asc('document_label'))->add(Restrictions::published())->setProjection(Projections::groupProperty('firstLetter', 'firstLetter'));
	...
	{
		$query = catalog_ProductService::getInstance()->createQuery()->add(Restrictions::eq('brand', $brand))->addOrder(Order::asc('label'));
		$query->createCriteria('compiledproduct')->add(Restrictions::published())->add(Restrictions::eq('brandId', $brand->getId()))->add(Restrictions::eq('shopId', $shop->getId()));
	...
	protected function preSave($document, $parentNodeId)
	{
		...
		if ($document->getPublicationstatus() === f_persistentdocument_PersistentDocument::STATUS_PUBLISHED && $document->isPropertyModified('label'))
		{
	...
	{
		if ($document->getPublicationstatus() === f_persistentdocument_PersistentDocument::STATUS_PUBLISHED || $oldPublicationStatus === f_persistentdocument_PersistentDocument::STATUS_PUBLISHED)
		{
			catalog_ProductService::getInstance()->setNeedCompileForBrand($document);
	...
	public function getResume($document, $forModuleName, $allowedSections = null)
	{
		...
		try
		{
			...
			$query = $this->createQuery()->add(Restrictions::eq('id', $document->getId()));
			
			$query->createPropertyCriteria('brandId', 'modules_catalog/compiledproduct')->add(Restrictions::eq('lang', $lang))->setProjection(Projections::groupProperty('websiteId', 'websiteId'));
	...
	{
		$query = $this->createQuery()->add(Restrictions::published())->add(Restrictions::eq('id', $brand->getId()))->setProjection(Projections::groupProperty('id', 'brandId'));
		
		$query->createPropertyCriteria('brandId', 'modules_catalog/compiledproduct')->add(Restrictions::published())->setProjection(Projections::groupProperty('websiteId', 'websiteId'), Projections::groupProperty('lang', 'lang'));
		...
		foreach ($compiledByWebsiteId as $websiteId => $brandDatas)
		{
			...
			foreach ($brandDatas as $data)
			{
				...
				$websiteInfos['brands'][] = array('lang' => $lang, 'plublication' => $publication);
			}
			$result[] = $websiteInfos;
		}
		return $result;
	...
	{
		$query = catalog_ProductService::getInstance()->createQuery()->add(Restrictions::eq('brand', $brand));
